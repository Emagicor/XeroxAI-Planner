...
function FileSlot({
  ...
  return (
    <div className="flex-1 min-w-0">
      <p className="text-xs font-medium text-[#8B8A82] mb-1">{label}</p>
      <p className="text-[10px] text-[#8B8A82] mb-2">{hint}</p>
      ...
      {!fileName ? (
        <button
          ...
          className="w-full px-4 py-6 rounded-lg border border-dashed border-line bg-surface/40 text-sm text-[#8B8A82] hover:border-accent/40 hover:text-[#F0EEE8] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {placeholder}
        </button>
      ) : (
        <div className="flex items-start gap-3 p-3 rounded-lg border border-line/60 bg-surface/50">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm text-[#F0EEE8] truncate" title={fileName}>
              {fileName}
            </p>
            {fileSize != null && (
              <p className="text-xs text-[#8B8A82] font-mono mt-0.5">
            ...
            <div className="flex gap-3 mt-2">
              ...
              {onClear && (
                <button
                  ...
                  disabled={disabled}
                  onClick={onClear}
                  className="text-xs text-[#8B8A82] hover:text-red-400 disabled:opacity-40"
                >
                  Remove
                </button>
              )}
    ...
    <div className="rounded-xl border border-line bg-card/60 p-4 transition-colors">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {testCase.label && (
            <span className="text-xs text-[#F0EEE8]">{testCase.label}</span>
          )}
          {testCase.permanent && (
  ...
  if (casesLoading) {
    return (
      <div className="rounded-xl border border-line bg-card/85 p-10 text-center">
        <p className="text-sm text-[#8B8A82]">Loading saved test cases…</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-accent/20 bg-accent/5 px-4 py-3">
        <p className="text-xs text-[#C8C6BE]">
          <span className="text-[#F0EEE8]">Save to suite</span> creates{' '}
      ...
      {cases.length === 0 ? (
        <div
          ...
          onDrop={onBulkDrop}
        >
          <p className="text-lg font-medium text-[#F0EEE8] mb-1">
            Build your test suite
          </p>
          <p className="text-sm text-[#8B8A82] mb-6 max-w-md mx-auto">
            No saved cases yet. Add a test case, attach floor plan + ground truth, then click{' '}
            <span className="text-[#F0EEE8]">Save to suite</span> to persist it.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <button
              type="button"
              onClick={onAddEmptyCase}
              className="px-5 py-2.5 rounded-lg border border-line bg-surface text-sm text-[#F0EEE8] hover:border-accent/50"
            >
              Add test case
            </button>
            <button
        ...
        <>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-1">
            <p className="text-sm text-[#8B8A82]">
              <span className="text-[#F0EEE8] font-medium">{readyCount}</span> saved &amp; ready
              {cases.length !== readyCount && (
                <>
                  {' '}
                  · <span className="text-[#F0EEE8] font-medium">{cases.length}</span> total
                </>
              )}
            </p>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={loading}
                onClick={onAddEmptyCase}
                className="px-3 py-1.5 rounded-lg border border-line bg-surface text-xs text-[#F0EEE8] hover:border-accent/50 disabled:opacity-40"
              >
                + Add case
              </button>
              <button
